...
def GetBaseModeParam( field ):
    ''' 根据字段获取解释 '''
    myConfigParser = MyConfigParser()
    myConfigParser.read( BASE_MODE_PATH )

    for param in myConfigParser.options( BASE_MODE ):
        if param != field:
            continue
        return myConfigParser.get( BASE_MODE, field )
	...
	def Add_t_pp_fav_index( self ):
		for i in range( int( self.GetRecordNumForFavIndex() ) ): # 非F101的数量
			...
			for section in self.MyConfigParser.options( table_section ):
				content = "%-20s =\t %-12s\n" % ( section, GetBaseModeParam( section ) )
				self.dst_file.write( content )
		self.dst_file.write( '\n' )
...
def Usage():
	print "\n\t Usage: python CreateNewMode.py [1] [2] [3] [4] [5]"
	...
	print "\n\t Demo: python CreateNewMode.py HK F101_vc/3,F101_gs/2 3 mprods ../result/a.mode\n"
	sys.exit()
